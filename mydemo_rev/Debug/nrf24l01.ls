...
3595                     ; 119     GPIO_WriteLow(NRF24L01_CS_PORT,NRF24L01_CS_PIN);// Set CSN low, init SPI tranaction
3597  0002 4b08          	push	#8
3598  0004 ae500f        	ldw	x,#20495
3599  0007 cd0000        	call	_GPIO_WriteLow
3601  000a 84            	pop	a
3602                     ; 120     status = SPI_RW(reg);               // Select register to write to and read status byte
3604  000b 7b03          	ld	a,(OFST+1,sp)
3605  000d cd0000        	call	_SPI_RW
3607  0010 6b01          	ld	(OFST-1,sp),a
3608                     ; 122     for(byte_ctr=0;byte_ctr<bytes;byte_ctr++)
3610  0012 0f02          	clr	(OFST+0,sp)
3612  0014 2014          	jra	L3322
3613  0016               L7222:
3614                     ; 123         pBuf[byte_ctr] = SPI_RW(0);     
...
3721                     ; 137     SPI_Write_Buf(WRITE_REG+TX_ADDR,buf,5);   //写入5个字节的地址.   
...
3731                     ; 138     SPI_Read_Buf(TX_ADDR,buf,5);              //读出写入的地址    
...
3741                     ; 139     for(i=0;i<5;i++)
3743  002b 0f06          	clr	(OFST+0,sp)
3744  002d               L3622:
3745                     ; 140       if(buf[i]!=0xa5)
...
3775                     ; 143       return 1;                               //NRF24L01不在位 
3777  004c a601          	ld	a,#1
3779  004e 2001          	jra	L23
3780  0050               L3722:
3781                     ; 144     return 0;                                 //NRF24L01在位
3783  0050 4f            	clr	a
3785  0051               L23:
3787  0051 5b06          	addw	sp,#6
3788  0053 81            	ret
3836                     ; 165 unsigned char nRF24L01_RxPacket(unsigned char* rx_buf)
3836                     ; 166 {
...
3946                     ; 191   	SPI_Write_Buf(WRITE_REG+RX_ADDR_P0,(u8*)RX_ADDRESS,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  
...
4204                     ; 230 	GPIO_WriteLow(NRF24L01_CE_PORT,NRF24L01_CE_PIN);
4206  0002 4b04          	push	#4
4207  0004 ae500f        	ldw	x,#20495
4208  0007 cd0000        	call	_GPIO_WriteLow
4210  000a 84            	pop	a
4211                     ; 231   	NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF  
...
4220                     ; 232  	GPIO_WriteHigh(NRF24L01_CE_PORT,NRF24L01_CE_PIN);//启动发送	   
4222  0017 4b04          	push	#4
4223  0019 ae500f        	ldw	x,#20495
4224  001c cd0000        	call	_GPIO_WriteHigh
4226  001f 84            	pop	a
4228  0020               L3242:
4229                     ; 234 	while(GPIO_ReadInputPin( GPIOD, GPIO_PIN_4) != 0);//等待发送完成
...
4238                     ; 235 	sta=SPI_Read(STATUS);  //读取状态寄存器的值	   
4240  002d a607          	ld	a,#7
4241  002f cd0000        	call	_SPI_Read
4243  0032 6b01          	ld	(OFST+0,sp),a
4244                     ; 236 	SPI_RW_Reg(WRITE_REG+STATUS,sta); //清除TX_DS或MAX_RT中断标志
